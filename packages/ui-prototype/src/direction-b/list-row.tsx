...
const statusChip = {
  running: { text: "LIVE", className: "bg-accent-2 text-shell" },
  blocked: { text: "HELD", className: "bg-warn text-shell" },
  closed: { text: "DONE", className: "bg-panel-2 text-ink-dim" },
} as const;
  ...
  return (
    ...
    >
      ...
      <span className="flex flex-col items-end gap-1">
        <span className={cn("hpb-clip px-1.5 py-px font-display text-[11px] font-bold tracking-[0.14em]", s.className)}>
          {s.text}
        </span>
        {trailing !== undefined && (
