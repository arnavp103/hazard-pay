...
describe("per-unit desynchronisation", () => {
  ...
  it("replays the same seeded stream", () => {
    const a = { randomState: 99 };
    const b = { randomState: 99 };
    for (let i = 0; i < 20; i += 1) { expect(nextRandom(a)).toBe(nextRandom(b)); }
  });

  it("carries its cursor as a plain number, so a state can be serialised", () => {
    const cursor = { randomState: 7 };
    nextRandom(cursor);
    const shipped = JSON.parse(JSON.stringify(cursor)) as { randomState: number };
    expect(nextRandom(shipped)).toBe(nextRandom({ randomState: cursor.randomState }));
  });

  it("derives independent streams from the same seed", () => {
    // Per-unit streams: a draw for unit 1 must not shift unit 2's sequence.
    const seeds = new Set([0, 1, 2, 3, 4, 5].map((id) => mixSeed(4242, id, 9)));
    expect(seeds.size).toBe(6);
    const first = { randomState: mixSeed(4242, 1, 9) };
    const again = { randomState: mixSeed(4242, 1, 9) };
    nextRandom(first);
    nextRandom(first);
    expect(nextRandom({ randomState: mixSeed(4242, 2, 9) }))
      .not.toBe(nextRandom({ randomState: mixSeed(4242, 1, 9) }));
    expect(nextRandom(again)).not.toBe(nextRandom(first));
  });
});
