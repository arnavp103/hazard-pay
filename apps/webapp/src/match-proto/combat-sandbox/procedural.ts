/**
 * THROWAWAY SCAFFOLDING (#96, ported from #89): the procedural animation
 * layers, plus the sandbox's PRNG.
 *
 * Everything here is a pure function of its arguments, so it is testable in
 * node with no renderer (see procedural.test.ts) and deterministic for
 * capture. These layers carry the motion; a very small authored base supplies
 * only the parts code is bad at (weight, intent, contact timing):
 *
...
/**
 * The sandbox PRNG (mulberry32), carried as an explicit cursor rather than a
 * closure.
 *
 * This is the difference between a battle you can only replay and one you can
 * resume. The lane this was ported from held the generator as a closure on the
 * battle state, which made the state unserialisable — and map #95 needs the
 * state at a slice boundary to survive being stored and picked back up. A
 * number round-trips through JSON; a closure does not.
 */
export interface RandomCursor {
  randomState: number;
}

/** Normalises a seed into a cursor value. Zero is not a usable state. */
export function seedRandom(seed: number): number {
  return (seed | 0) || 1;
}

/**
 * Derives an independent stream seed from any number of parts.
 *
 * Use it to give each unit — and, if a behaviour needs isolation, each
 * *purpose* — its own stream: `mixSeed(seed, unit.id, PURPOSE)`. Sharing one
 * cursor across units couples them, so adding a single extra draw for unit 5
 * silently shifts every draw unit 6 makes afterwards. Per-unit streams mean a
 * new behaviour perturbs only the units it actually touches.
 */
...
/** Draws the next value in [0, 1) and advances the cursor in place. */
export function nextRandom(cursor: RandomCursor): number {
  const state = (cursor.randomState + 0x6d2b79f5) | 0;
  cursor.randomState = state;
